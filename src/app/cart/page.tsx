...
import CartItemAdding from './CartItemAdding'
import { IoIosCheckmarkCircle } from 'react-icons/io'
import Link from 'next/link'

const CartPage = () => {
    const images = ['/cart5ahmed.png', '/cart2ahmed.png', '/cart3ahmed.png', '/cart4ahmed.png', '/cart5ahmed.png']
    return (
                ...
                <div className='grid grid-cols-4 lg:grid-cols-6 items-start gap-3'>
                    <div className='col-span-4 flex flex-col gap-4'>
                        <table className='overflow-x-scroll'>
                            <thead>
                                <tr className='font-poppins text-[#1D3178] font-bold text-xl text-center'>
                            ...
                            <tbody>
                                {images.map((image, index) => (
                                    <CartItemAdding key={index} image={image} />
                                ))}
                            </tbody>
                        </table>
                        <div className='flex justify-between items-center'>
                            <button className='bg-pinkcolor rounded-sm font-poppins px-6 text-white py-3'>Update Cart</button>
                            <button className='bg-pinkcolor rounded-sm font-poppins px-6 text-white py-3'>Clear Cart</button>
                        </div>
                    </div>
                    <div className="flex flex-col md:flex-row lg:flex-col gap-4 col-span-full md:gap-3 lg:col-span-2">
                        <div className='flex flex-col md:col-span-2 lg:col-span-2'>
                            ...
                            <div className='bg-[#F4F4FC] rounded-sm p-4'>
                                ...
                                <div className='flex justify-between items-center pt-10 pb-2 border-b border-gray-400 py-4'>
                                    ...
                                    <p className='font-lato text-lg text-navyBlue'>€325.00</p>
                                </div>
                        ...
                        <div className='flex flex-col md:col-span-2 lg:col-span-2'>
                            ...
                            <div className='bg-[#F4F4FC] rounded-sm p-4 flex flex-col gap-y-8'>
                                <input type="text" className='bg-transparent text-gray-600 placeholder:text-[#C5CBE3] border-b border-b-[#C5CBE3] outline-none pb-2' placeholder='Bangladesh' />
                                <input type="text" className='bg-transparent text-gray-600 placeholder:text-[#C5CBE3]  border-b border-b-[#C5CBE3] outline-none pb-2' placeholder='Mirapur Dhaka, 1200 ' />
                                <input type="text" className='bg-transparent text-gray-600 placeholder:text-[#C5CBE3]  border-b border-b-[#C5CBE3] outline-none  pb-2' placeholder='Postal Code' />
                                <div className='flex justify-start gap-1 py-6'>
                                    <button className='bg-pinkcolor rounded-sm font-poppins px-8 text-white py-3'>Calculate Shipping</button>
                                </div>
                            </div>
                        </div>
                    </div>
